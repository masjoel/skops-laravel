    ...
    <div class="row justify-content-center">
        <div class="col-12 col-md-5">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('master.kelas.update', $kelas->id) }}">
                        @csrf @method('PUT')
                        <div class="mb-3">
                            <label class="form-label">Nama Kelas <span class="text-danger">*</span></label>
                            <input type="text" name="nama_kelas" class="form-control @error('nama_kelas') is-invalid @enderror"
                                value="{{ old('nama_kelas', $kelas->nama_kelas) }}" placeholder="Contoh: 7A, 8B, 9C" oninput="this.value = this.value.toUpperCase();" required>
                            @error('nama_kelas')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="tingkat" class="form-control @error('tingkat') is-invalid @enderror"
                                value="{{ old('tingkat', $kelas->tingkat) }}" placeholder="Contoh: 7, 8, 9, X, XI, XII" oninput="this.value = this.value.toUpperCase();" required>
                            @error('tingkat')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select name="jurusan_id" class="form-control @error('jurusan_id') is-invalid @enderror">
                                ...
                                @foreach ($jurusan as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('jurusan_id', $kelas->jurusan_id) == $item->id ? 'selected' : '' }}>
                                        {{ $item->nama }}
                                    </option>
                                @endforeach
                            </select>
