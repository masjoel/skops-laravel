                    ...
                    <div class="col-auto">
                        @if ($sekolah->logo)
                            <img src="{{ asset('storage/' . $sekolah->logo) }}"
                                style="height:60px;border-radius:8px;object-fit:contain">
                        @else
                            <div
                                style="width:60px;height:60px;background:rgba(99,102,241,.1);border-radius:8px;display:flex;align-items:center;justify-content:center">
                                <i class="fas fa-building fa-lg" style="color:#6366f1"></i></div>
                        @endif
                    </div>
                    <div>
                        <div style="font-size:18px;font-weight:700;color:var(--text-primary)">
                            {{ $sekolah->nama_client }}</div>
                            <div style="font-size:13px;color:var(--text-muted)">NPSN:
                                {{ $sekolah->npsn }}<br>{{ $sekolah->alamat_client }}<br>{{ $sekolah->kota }}</div>
                            <div style="font-size:13px;color:var(--text-muted)">Telp: {{ $sekolah->telpon }} <br>Email:
                                {{ $sekolah->email ?? '' }}</div>
                        </div>
                    </div>
                </div>
